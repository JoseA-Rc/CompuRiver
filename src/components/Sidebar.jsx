...
const Sidebar = () => {
  ...
  return (
    <div className={`${isOpen ? 'right-0' : '-right-full'} w-full bg-white fixed top-0 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      ...
      <div className='flex flex-col gap-y-1 h-[650px] lg:h-[430px] overflow-y-auto overflow-x-hidden border-b'>
        {cart.map((item) => {
      ...
      <div className='flex flex-col gap-y-3 py-4 mt-4'>
        <div className='flex w-full justify-between items-center'>
          ...
          {/* icono limpiar carrito */}
          <div onClick={() => clearCart()} className='cursor-pointer py-4 bg-red-500 text-white w-12 h-12 flex justify-center items-center text:xl'>
            <FiTrash2/>
          </div>
        </div>
        <Link to='/' className='bg-black flex p-1 justify-center items-center text-white w-full font-medium'>
          Pagar
        </Link>
      </div>
    </div>
